...
#[derive(Debug)]
pub struct Matrix<T> where T: Numeric{
    rows: usize,
    cols: usize,
    data: Vec<T>,
}
...
impl<T> Matrix<T> where T: Numeric {
    ...
    pub fn copy_to<S>(&self) -> Result<Matrix<S>, MatrixCopyToError> where S: Numeric {
        MatrixBuilder::<S>::new()
        .from_vec(
            self.rows, 
            self.cols, 
...
impl<T> ops::Index<(usize, usize)> for Matrix<T> where T: Numeric {
    type Output = T;
...
impl<T> ops::IndexMut<(usize, usize)> for Matrix<T> where T: Numeric {
...
impl<T> PartialEq for Matrix<T> where T: Numeric {
...
impl<T> fmt::Display for Matrix<T> where T: Numeric {
...
impl<T> Swap for Matrix<T> where T: Numeric {
    fn swap_cols(&mut self, j1: usize, j2: usize) -> Result<(), MatrixIndexOutOfBoundsError> {
...
impl<T> MatrixBuilder<T> where T: Numeric {
    pub fn new() -> Self {
        MatrixBuilder {
            rows: None,
            cols: None,
    ...
    pub fn zeros(self, rows: usize, cols: usize) -> Self {
        self.rows(rows).cols(cols).data(vec![T::default(); rows * cols])
    }

    pub fn identity(self, n: usize) -> Self {
        let mut data = vec![T::default(); n * n];
        for k in 0..n { data[k * n + k] = T::one(); }
